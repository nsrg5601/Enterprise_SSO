// src/components/Employee/EmployeeLogin.js
...
import { EMPLOYEE_TOKEN_KEY, AUTH_TOKEN_KEY } from '../../config';
...
const EmployeeLogin = ({ setIsLoggedIn }) => {
  ...
  const handleSSOLogin = async () => {
    try {
      ...
      const response = await axios.post(
        'http://localhost:8081/employee/sso-login',
        {},
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        localStorage.setItem(EMPLOYEE_TOKEN_KEY, newToken); // Update JWT token
        setIsLoggedIn(true);
